---
name: huashu-design
description: 花叔Design — 用HTML做高保真原型、幻灯片、动画、设计变体、设计方向顾问。主干：Junior Designer工作流、反AI slop、React+Babel最佳实践、Starter Components、Playwright验证、动画→MP4/GIF导出、带解说长动画pipeline。触发电：原型/动画/设计探索/视频/评审。
---
...
| **无桌面环境（WSL/Headless Server）需 PDF 导出** | 运行在 WSL、远程 VPS 或没有 Chrome 的环境 | 不要死磕命令行工具。走兜底方案：生成含 @page 规则的 HTML → 复制到用户桌面 → Chrome 打印（Ctrl+P→另存为PDF）。详见 `references/pdf-export-wsl.md` |
...
| **HTML → PDF 导出（环境受限）** | `references/pdf-export-fallback.md` — Chrome Ctrl+P 兜底方案 |
| **设计评审/打分**（设计完成后可选） | `references/critique-guide.md`（5 维度评分+常见问题清单） |
| **WSL 环境导出 PDF**（无 Chrome/Playwright 时） | `references/wsl-pdf-export.md`（Windows Chrome Ctrl+P方案） |
| **PDF 导出失败兜底（WSL/无桌面环境）** | `references/pdf-export-wsl.md`（Chrome 打印替代方案） |
...
- **WSL 环境 PDF 导出**：`references/wsl-pdf-export.md` — Ubuntu 26.04+ 无 Chrome/无 sudo 时，复制 HTML 到 Windows 侧用 Chrome Ctrl+P → 另存为 PDF
