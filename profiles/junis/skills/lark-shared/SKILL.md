...
### 方式二：在 Hermes Agent 环境中绑定

在 Hermes Gateway 环境中首次运行 lark-cli 时，可能会遇到：

```
"hermes context detected but lark-cli is not bound to it"
```

这说明 lark-cli 还未与当前 Hermes profile 绑定。执行：

```bash
lark-cli config bind --source hermes --identity bot-only --app-id <APP_ID>
```

绑定成功后输出 `"ok": true, "message": "已绑定应用 <APP_ID> 到 Hermes"`，后续所有命令无需再指定 `--as` 就可以用 bot 身份操作。

**注意：** `--source` 需要根据实际环境指定（hermes / openclaw / lark-channel），`--identity` 可选 `bot-only` 或 `user-default`。这是一次性同步，后续 Agent 配置变更不会自动更新到 lark-cli。如需重新同步，请执行 `lark-cli config bind`。
