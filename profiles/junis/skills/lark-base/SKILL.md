...
> **复杂Excel导入：** 如果Excel结构非标（并列双栏/分类台账/合并行），`drive +import` 会原样导入混乱结构。此时走 [`references/excel-to-base-pipeline.md`](references/excel-to-base-pipeline.md) 管线：Python解析变换 → 手动建表字段 → 灌数据 → 建仪表盘。
> **财务数据自动汇总：** 如果用户需要日常记账+自动汇总+可视化复盘，走 [`references/financial-summary-base-architecture.md`](references/financial-summary-base-architecture.md) 三表架构：明细(录入) + 汇总(Lookup+Formula自动计算) + 仪表盘。
...
| 本地 Excel / CSV / `.base` 导入为 Base | `lark-cli drive +import --type bitable` | 不要误走 `+base-create`、`+table-create` 或 `+record-upsert` |
| 非规整Excel（嵌套结构/双栏并列/分类台账）导入 Base | 用Python解析+变换 → `+base-create` → `+field-create`(逐字段) → `+record-batch-create` | 不要直接 `drive +import`（会原样导入混乱结构）；不要用 `+table-create --fields`（参数验证失败时会静默创建无字段的空表） |
| 财务台账（收入/支出流水）需要自动汇总+可视化看板 | 走 [`references/financial-summary-base-architecture.md`](references/financial-summary-base-architecture.md) 三层架构：明细表(录入) → Link到汇总表 → Lookup聚合 → Formula计算结余 → 仪表盘可视化 | 不要把所有字段放在一张表里然后手动算；Link+Lookup+Formula才是Base的原生自动计算方案 |
...
本用户工作区有三张关联的多维表格，**写搭建日志时必须同步检查另外两张表**。详情见 `references/三表同步工作流.md`。

| 表 | 定位 | 表ID |
|----|------|------|
| 搭建日志 | 每次操作一条记录 | `tbllV9WgN64Zwput` |
| 每日记录 | 每日一条汇总 | `tblTLllADiUdhL6e` |
| 收藏随想录 | 收藏内容 | `tblydJHMALlK3stv` |
...
涉及检查搭建日志（Build Log）表完整性时，先阅读 [`references/build-log-audit-sop.md`](references/build-log-audit-sop.md)。审计检查项包括：空状态字段、日期断层、跨会话任务追溯、状态一致性。

**⚠️ 关键规则：写搭建日志后，必须同步更新「每日记录」和「收藏随想录」——三表一体，不可只写一张表。** 具体字段映射和同步流程见 SOP 中「三表同步」章节。
...
| `+table-create --fields` 返回 `ok=false` 但表已创建 | `--fields` 中某字段参数验证失败时，表仍被创建（仅含默认自增ID字段），但字段不会回滚 | 先 `+table-list` 确认；要么删除重试，要么改用 `+field-create` 逐个建字段。**建议：建表时先不传 `--fields`，用 `+field-create` 分步建字段，更可控** |
| `--json @file` 要求相对路径 | `lark-cli base +record-batch-create --json @/absolute/path` 报错："--file must be a relative path within the current directory" | 先把JSON文件 `cp` 到当前工作目录，再用 `--json @./filename.json` |
| strict mode = bot 时无法切换用户身份 | `lark-cli auth login` 被拒绝，提示 "strict mode is bot" | 所有操作以 bot 身份执行；Base创建后需手动在飞书UI中为团队成员授权（bot无法自动grant） |
