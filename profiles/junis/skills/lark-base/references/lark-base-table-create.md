...
- ⚠️ 不要并行改同一张表，避免状态竞争。
- ⚠️ **`--fields` 中任一字段参数验证失败 → 全部字段被跳过，但表仍被创建。** 这是静默的部分失败：`ok=false` 返回了错误，但表已提交（仅含默认自增ID字段）。后续 `+table-list` 能看到该表但无自定义字段。恢复：先 `+field-list` 确认字段状态，要么 `+table-delete` 重试，要么改用 `+field-create` 逐个补建字段。**推荐：一次性建多个字段时，先单独 `--name` 建空表，再用 `+field-create`（串行+0.5s延迟）逐个建字段，更可控。**
