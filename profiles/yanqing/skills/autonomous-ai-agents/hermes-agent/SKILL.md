...
**Creating custom MCP services:** You can implement MCP servers in Python (no Node.js required). See `references/creating-python-mcp-services.md` for the full guide.
...
For profiles that need a primary model with a fallback provider, see the verified config template in `references/multi-provider-fallback-config.md`. Key points:

- Each profile must declare the stepfun provider in its own `providers` block — `.env` values do NOT inherit across profiles
- The `fallback_providers` block requires `activation: mode: sequential` + `min_priority: 1` to engage on primary failure
- Profile inventory (4 profiles): junis, huanglaoxie, saodiseng use deepseek-v4-flash → step-3.5-flash; yanqing uses step-3.7-flash as primary (no fallback)
...
- **Per-job knobs:** `skills`, `model`/`provider` override, `script`
  ...
  ⚠️ **Script path resolution pitfall:** The cron system resolves
  `script` as `<workdir>/scripts/<script>`. If you set
  `script: "scripts/foo.py"` and `workdir: "/home/hermes/.hermes/profiles/yanqing"`,
  the resolved path becomes
  `...yanqing/scripts/scripts/foo.py` — **double `scripts/`**. Use
  just the filename: `script: "foo.py"` → resolves to
  `<workdir>/scripts/foo.py` correctly.
...
**`pip install` fails with "externally-managed-environment" on WSL.** 
Debian PEP 668 blocks pip to system site-packages. Use the project's
Python binary with `--break-system-packages`:
`python3.14 -m pip install --break-system-packages <pkg>`. Full pattern:
`references/python-pip-break-system-packages.md`.
...
**Auxiliary title generation fails with 401 ("Incorrect API key provided")** — see `references/auxiliary-client-401-stale-base-url.md`. Root cause is a stale `OPENAI_BASE_URL` in `.env` pointing at a different provider than the configured `model.provider`. The auxiliary client reads `OPENAI_BASE_URL`/`OPENAI_API_KEY` directly from `.env`, so a leftover DashScope/TokenHub value overrides the correct provider routing.
